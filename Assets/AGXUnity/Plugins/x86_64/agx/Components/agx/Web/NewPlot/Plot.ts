...
$( function() {
  ...
  function init(plot) {
    ...
    plot.hooks.processOptions.push( (plot, options) => {
      ...
      {
        plot.hooks.drawBackground.push( (plot, ctx) => {
...
interface FlotWindowMap {
    [id: string]: agx.plot.FlotWindow;
}
...
class WebSocketHandler
{
  ...
  promiseTable : any;
  plotWindows: FlotWindowMap;

  hasResizeRequest = false;
  isThrottled = false;
  maxResizeFrequency : number = 30;
  hasBeenOpened = false;
  hasEmptyWindow = false;

  static EMPTY_WINDOW_NAME : string = "   Figure   ";
...
/*    console.log(JSON.stringify(header, undefined, 2));*/
    ...
    else if (message.uri == "Plot.DescriptionPacket")
    {
      ...
      for (var i = 0; i < windows.length; i++)
      {
        ...
        {
          var shouldAddCurve : boolean = false;
  ...
  onContextMenuItemClicked(windowName : string, key : string)
  {
...
function initFlot(label: string, name: string)
{
