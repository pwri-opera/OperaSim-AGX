// This file is based on a copy of the original file AGXUnity/Rendering/DeformableTerrainParticleRenderer.cs 
// of AGX Dynamics For Unity 2.4.2, with the following modifications applied:
//   * Fixes bug that caused some particles to not render when particle count was greater than 1023.
//   * Applies shadowCastingMode and receiveShadows from source MeshRenderer.
//   * Option to synchronize transformations from Update() instead of once per physics step.
// Copyright 2021 VMC Motion Technologies Co., Ltd. on modified code.
...
{
  [AddComponentMenu( "AGXUnity/Deformable Terrain Particle Renderer Extended" )]
  [RequireComponent( typeof( DeformableTerrain ) )]
  public class DeformableTerrainParticleRendererExt : ScriptComponent
  {
    ...
    private bool InitializeRenderMode()
    {
      ...
      if ( GranuleInstance == null ) {
        Debug.LogError( "AGXUnity.Rendering.DeformableTerrainParticleRendererExt: " +
    ...
    private void Synchronize()
    {
      ...
      if ( isValidDrawInstanceMode ) {
        ...
        while (m_numGranulars / 1023 + 1 > m_granuleMatrices.Count)
        {
            ...
            Debug.Log($"Added array to m_granuleMatrices. Num arrays = {m_granuleMatrices.Count}");
        }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(DeformableTerrainParticleRendererExt))]
    [CanEditMultipleObjects]
    public class AGXUnityRenderingDeformableTerrainParticleRendererExtEditor : InspectorEditor
    { }
#endif
}
